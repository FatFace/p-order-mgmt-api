...
---
{
	orders: vars.originalPayload.'pick-orders' map ( pickorder , indexOfPickorder ) -> {
		...
		"order-type":if((pickorder."route-id") != null) (if(sizeOf(pickorder."route-id")>=20) ((pickorder."route-id")[0 to 19]) else (pickorder."route-id")) else "",
		"order-date": if((pickorder."order-date") != null) (if(sizeOf(pickorder."order-date")>=14) ((pickorder."order-date")[0 to 13]) else (pickorder."order-date")) else "",
		...
		"ship-by-date": if((pickorder."delivery-date") != null ) (if(sizeOf(pickorder."delivery-date")>=14) ((pickorder."delivery-date")[0 to 13]) else (pickorder."delivery-date")) else "",
		...
		"inv-name": if((pickorder.customer.fullname) != null) (if(sizeOf(pickorder.customer.fullname)>= 50) ((pickorder.customer.fullname)[0 to 49]) else (pickorder.customer.fullname)) else "",
		"inv-contact-address1": if((pickorder.customer.fullname) != null) (if(sizeOf(pickorder.customer."billing-address"."address-1")>=60) ((pickorder.customer."billing-address"."address-1")[0 to 59]) else(pickorder.customer."billing-address"."address-1")) else "",
		...
		"order-lines": pickorder.orderlines map ( orderline , indexOfOrderline ) -> {
			...
			"product-barcode": if((orderline.barcode) != null) (if(sizeOf(orderline.barcode)>=50) ((orderline.barcode)[0 to 49]) else (orderline.barcode)) else "",
			...
			"product-name": if((orderline."item-name") != null) (if(sizeOf(orderline."item-name")>30) ((orderline."sales-price")[0 to 29]) else (orderline."sales-price")) else "",
